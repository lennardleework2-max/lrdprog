...
$select_db_xtotal = "SELECT count(*) as rec_count FROM tranfile1 LEFT JOIN supplierfile ON tranfile1.suppcde = supplierfile.suppcde WHERE true AND trncde='".$trncde."' ".$xfilter;
...
$select_db="SELECT tranfile1.shipto as tranfile1_shipto,tranfile1.docnum as tranfile1_docnum,tranfile1.trndte as tranfile1_trndte,tranfile1.trntot as tranfile1_trntot,
tranfile1.orderby as tranfile1_orderby,tranfile1.recid as tranfile1_recid, supplierfile.suppdsc as supplierfile_suppdsc, tranfile1.paydate as tranfile1_paydate, tranfile1.ordernum as tranfile1_ordernum  FROM tranfile1 LEFT JOIN supplierfile ON 
tranfile1.suppcde = supplierfile.suppcde WHERE true AND trncde='".$trncde."' ".$xfilter." LIMIT ".$xlimit." OFFSET ".$xoffset;
$xret["sql"] = $select_db;
...
while($row_main = $stmt->fetch()){
    ...
    if(empty($row_main['supplierfile_suppdsc'])){
    ...
    $hide_price = false;
    if(isset($_POST['crud_hide_hidden'])  &&  $_POST['crud_hide_hidden'] == "1"){
        $row_main['tranfile1_paydate'] = '&nbsp';
        $hide_price = true;
    }
