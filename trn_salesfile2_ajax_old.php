<?php
    ...
    if(isset($_POST["event_action"]) && $_POST["event_action"] == "getEdit"){
        ...
        $select_tranfile2="SELECT tranfile2.wholesaleprc as wholesaleprc, tranfile2.itmcde, tranfile2.itmqty, tranfile2.untprc, tranfile2.extprc, itemfile.itmdsc, tranfile2.recid as tranfile2_recid FROM tranfile2 LEFT JOIN itemfile ON itemfile.itmcde = tranfile2.itmcde WHERE tranfile2.recid=?";
        ...
        $xret["retEdit"] = [
            "itmcde" =>  $rs_tranfile2["itmcde"],  // jomer 5-8-2025
    ...
    if(isset($_POST["event_action"]) && $_POST["event_action"] == "getCurrentStock"){
        ...
        $select_db2 = "SELECT SUM(stkqty) as xsum FROM tranfile2 
            ...
            WHERE tranfile2.itmcde='".$_POST['current_itmcde']."' 
            AND tranfile1.trndte<='".$date_today."'";
        $stmt_main2	= $link->prepare($select_db2);
        $stmt_main2->execute(array());
        $rs2 = $stmt_main2->fetch();

        $xret["currentStockToday"] = $rs2['xsum'];

        $select_itemfile2="SELECT * FROM itemfile WHERE itmcde='".$_POST['current_itmcde']."'";
        $stmt_itemfile2	= $link->prepare($select_itemfile2);
        $stmt_itemfile2->execute();
        $rs_itemfile2 = $stmt_itemfile2->fetch();
        $xret['wholesaleprc'] = $rs_itemfile2['wholesaleprc'];
    ...
    $xret["html"] .="<div class='w-100 d-flex justify-content-center'>";
        $xret["html"] .= "<table class='table table-striped' style='width:90%'>";
            ...
            $select_salesfile2="SELECT tranfile2.wholesaleprc as wholesaleprc, itemfile.itmdsc as itemfile_itmdsc,itemfile.itmcde as itemfile_itmcde, tranfile2.itmqty, tranfile2.untprc, tranfile2.extprc, tranfile2.recid as tranfile2_recid
