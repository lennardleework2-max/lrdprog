<?php
    ...
    //$select_db_cus_route="SELECT *, mf_routes.route_desc as 'route_desc', mf_routes.route_id as 'route_id', mf_buyers.buyer_name as 'buyer_name'  FROM mf_buyers LEFT JOIN mf_routes ON mf_buyers.route_id = mf_routes.route_id WHERE mf_buyers.buyer_id='".$_POST['cus_add_hidden']."' LIMIT 1";
    $select_db_cus_route="SELECT *, mf_routes.route_desc as 'route_desc', mf_routes.route_id as 'route_id', mf_buyers.buyer_name as 'buyer_name'  FROM mf_buyers LEFT JOIN mf_routes ON mf_buyers.route_id = mf_routes.route_id WHERE mf_routes.route_id='".$_POST['route_id_hidden']."' LIMIT 1";
    $stmt_main_cus_route	= $link->prepare($select_db_cus_route);
    $stmt_main_cus_route->execute();
    $rs_main_cus_route = $stmt_main_cus_route->fetch();


    $select_db_cus="SELECT *, mf_routes.route_desc as 'route_desc', mf_routes.route_id as 'route_id', mf_buyers.buyer_name as 'buyer_name'  FROM mf_buyers LEFT JOIN mf_routes ON mf_buyers.route_id = mf_routes.route_id WHERE mf_buyers.buyer_id='".$_POST['cus_add_hidden']."' LIMIT 1";
    $stmt_main_cus	= $link->prepare($select_db_cus);
    $stmt_main_cus->execute();
    $rs_main_cus = $stmt_main_cus->fetch();


    $logoPath = 'images/ryu_motor_logo.jpg';   // relative to project root
    $title    = 'COUNTER RECEIPT 12';
    ...
    // Light gray route descriptor
    $pdf->setColor(0.7, 0.7, 0.7);
    $pdf->addText($pw - $rm - $tw - 20, $topY - 14, 14, $rs_main_cus_route['route_desc']);
    
    // Restore red for main title
    $pdf->setColor(0.85, 0.10, 0.10);
    $pdf->addText($pw - $rm - $tw - 20, $topY - 34, 14, $title);
    $pdf->restoreState();
    ...
    $pdf->ezPlaceData($xleft+75,$xtop,$rs_main_cus['buyer_name'],13,'left');
    $xtop-=18;
   ...
   // $pdf->ezPlaceData($xleft,$xtop,"0917-700-0224",13,'left');
    ...
    $fields = '';
        ...
        // Footer-like notice on FIRST page only (bottom area)
        $firstFooter = $pdf->openObject();
        $pdf->saveState();

        $footerLeft = $lm;                      // align to margins
        $footerWidth = $pw - $lm - $rm;

        // Bank info boxed block (taller, extends from bottom a bit)
        $bankH = 44;                            // increased height for more padding
        $bankY = 82;                            // start slightly lower to extend from bottom
        $pdf->setColor(0,0,0,'stroke');
        $pdf->rectangle($footerLeft, $bankY, $footerWidth, $bankH);

        $lineY1 = $bankY + $bankH - 12;
        $lineY2 = $lineY1 - 12;
        $lineY3 = $lineY2 - 12;

        $labelX = $footerLeft + 8;  $valueX = $labelX + 90;
        $pdf->setColor(0.85,0.10,0.10);  $pdf->addText($labelX,$lineY1,9,'BANK:');
        $pdf->setColor(0,0,0);          $pdf->addText($valueX,$lineY1,9,'RCBC');

        $pdf->setColor(0.85,0.10,0.10);  $pdf->addText($labelX,$lineY2,9,'BANK ACCOUNT:');
        $pdf->setColor(0,0,0);           $pdf->addText($valueX,$lineY2,9,'ONE PACIFIC GLOBAL VENTURES CORPORATION');

        $pdf->setColor(0.85,0.10,0.10);  $pdf->addText($labelX,$lineY3,9,'ACCOUNT NUMBER:');
        $pdf->setColor(0,0,0);           $pdf->addText($valueX,$lineY3,9,'0000007591213241');

        // Thank you centered message (reduced gap)
        $pdf->setColor(0,0,0);
        $pdf->ezPlaceData($footerLeft + ($footerWidth/2), 60, '<b>THANK YOU FOR YOUR BUSINESS!</b>', 10, 'center');

        // Contact lines (compact vertical spacing)
        $lx = $footerLeft;            $rx = $footerLeft + ($footerWidth/2) - 40; // bring right column ~44px closer
        $yA = 42;                     $yB = 29;
        $labelPadL = 44;              $labelPadR = 42; // reduced label-to-value padding
        // Left column
        $pdf->setColor(0.85,0.10,0.10); $pdf->addText($lx, $yA, 9, 'Company:');
        $pdf->setColor(0,0,0);         $pdf->addText($lx+$labelPadL, $yA, 9, 'One Pacific Global Ventures Corp.');
        $pdf->setColor(0.85,0.10,0.10); $pdf->addText($lx, $yB, 9, 'Tel:');
        $pdf->setColor(0,0,0);         $pdf->addText($lx+$labelPadL, $yB, 9, '0916-433-8621');
        // Right column
        $pdf->setColor(0.85,0.10,0.10); $pdf->addText($rx, $yA, 9, 'Email:');
        $pdf->setColor(0,0,0);         $pdf->addText($rx+$labelPadR, $yA, 9, 'marketing.megaone@gmail.com');
        $pdf->setColor(0.85,0.10,0.10); $pdf->addText($rx, $yB, 9, 'Address:');
        $pdf->setColor(0,0,0);         $pdf->addText($rx+$labelPadR, $yB, 9, 'SteelWorld Tower Unit 805, NS Amoranto cor Biak na Bato, QC');

        $pdf->restoreState();
        $pdf->closeObject();
        $pdf->addObject($firstFooter,'add');
    ...
    if(isset($_POST['cus_add_hidden']) && !empty($_POST['cus_add_hidden'])){
        ...
        //$xfilter .= " AND tranfile1.route_id='".$rs_main_cus_route['route_id']."'";
    }
    ...
    $select_db = "SELECT * FROM tranfile1 WHERE true ".$xfilter." AND (paydate IS NULL OR paydate = '') AND salesman_id!='".$rs_main_empty_salesman['salesman_id']."' AND tranfile1.trncde='SAL'  "; 
    ...
    $stmt_main->execute();
    $grand_total = 0; // accumulate TOTAL column
    ...
    while($rs_main = $stmt_main->fetch()){    
        ...
        // accumulate per row total
        if (isset($rs_main["trntot"])) {
            $grand_total += floatval($rs_main["trntot"]);
        }
        ...
        $pdf->setColor(0, 0, 0);
        $ty = $rowY + 5;                       // baseline inside row (compact)
        $fs = 10;                              // font size
        $trndte_disp = '';
        ...
        $xtop = $rowY - 1;                     // smaller gap between rows
        ...
        if($xtop <= 150)
        {
            ...
            $barW = $pw - $rm - $barX;              // dynamic width based on portrait/landscape
            $barH = 22;                             // header bar height
            $barY = 750 - $barH;                  // rectangle uses bottom-left origin
    ...
    // After listing rows, render a TOTAL AMOUNT row
    // Respect the same column layout used above
    $xleft = 30;
    $sumRowH = 18;
    $rowY  = $xtop - $sumRowH;                // position directly below last row
    $tblX  = $xleft;
    $tblW  = $pw - $rm - $tblX;
    ...
    $colW1 = (int)round($tblW * 0.24); // DATE
    $colW2 = (int)round($tblW * 0.26); // DR NO.
    $colW3 = $tblW - ($colW0 + $colW1 + $colW2); // TOTAL (remainder)
    $cX0 = $tblX;
    $cX1 = $cX0 + $colW0;
    $cX2 = $cX1 + $colW1;
    $cX3 = $cX2 + $colW2;

    // If we're too close to the bottom, push to a new page and reset Y
    if ($rowY <= 150) {
        $pdf->ezNewPage();
        $rowY = 725 - $sumRowH; // align like the reflow logic above
    }

    // No borders for the total row (clean look)

    // Fill last cell red and print white total value right-aligned
    $pdf->setColor(0.85, 0.10, 0.10, 'fill');
    $pdf->filledRectangle($cX3, $rowY, $colW3, $sumRowH);

    $pdf->setColor(1, 1, 1); // white text for amount
    $pdf->ezPlaceData($cX3 + $colW3 - 6, $rowY + 5, '<b>' . number_format($grand_total, 2) . '</b>', 10, 'right');

    // Label moved one column to the right (center of column 3)
    $pdf->setColor(0, 0, 0); // black text
    $pdf->ezPlaceData($cX2 + ($colW2/2), $rowY + 5, '<b>TOTAL AMOUNT</b>', 10, 'center');
