...
ini_set('memory_limit',-1);
    ...
    $xprog_module_log = 'TOP SALES SALESMAN';
    $xactivity_log = $is_tab_export ? 'export_txt' : 'export_pdf';
    $xremarks_log = "Exported ".$export_label." from Top Sales Salesman";
    ...
    (isset($_POST['date_to']) && !empty($_POST['date_to'])) ){
        ...
        $pdf->saveState();
        $pdf->ezPlaceData($xleft, $xtop,"<b>Top Sales Report (by Salesman)</b>", 15, 'left' );
        $xtop   -= 15;
        ...
        $pdf->ezPlaceData($xleft,$xtop,"<b>Salesman</b>",10,'left');
...
*/
$select_db="SELECT salesman_name,sum(trntot) as tot_trntot
    from tranfile1
    LEFT JOIN mf_salesman ON tranfile1.salesman_id=mf_salesman.salesman_id
    WHERE tranfile1.trncde='SAL'  ".$xfilter." GROUP BY tranfile1.salesman_id ORDER BY tot_trntot DESC";
  //  var_dump($select_db);
  //  die();
    ...
    while($rs_main = $stmt_main->fetch()){
        ...
        $pdf->ezPlaceData($xleft,$xtop,trim_str($rs_main["salesman_name"],300,9)??'',9,"left");
