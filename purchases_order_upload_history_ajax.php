...
if(isset($_POST['ordernum_search_h']) && trim($_POST['ordernum_search_h']) !== ''){
    $xfilter .= " AND ordernum LIKE ?";
    $params[] = '%' . trim($_POST['ordernum_search_h']) . '%';
}

// Filter by docnum
if(isset($_POST['docnum_search_h']) && trim($_POST['docnum_search_h']) !== ''){
    $xfilter .= " AND docnum LIKE ?";
    $params[] = '%' . trim($_POST['docnum_search_h']) . '%';
}
...
$select_count = "SELECT COUNT(DISTINCT file_name) as rec_count FROM po_upld_history WHERE 1=1 " . $xfilter;
...
// Handle no records
if($xtotalrec == 0){
    $xret["html"] = "<tr><td colspan='6' class='text-center py-4'><div class='display-6'>No Records Found <i class='fas fa-search ms-2'></i></div></td></tr>";
...
// Join with users table to get userdesc
$select_data = "SELECT h.file_name,
                       MAX(h.date_uploaded) as date_uploaded,
                       MAX(h.ordernum) as ordernum,
                       MAX(h.docnum) as docnum,
                       MAX(h.usercode) as usercode,
                       COUNT(*) as record_count
                FROM po_upld_history h
                WHERE 1=1 " . $xfilter . "
                GROUP BY h.file_name
                ORDER BY MAX(h.date_uploaded) DESC
...
while($row = $stmt_data->fetch()){
    ...
    $docnum = htmlspecialchars($row['docnum'] ?? '', ENT_QUOTES);
    ...
    // Get userdesc from users table
    $userdesc = '';
    if(!empty($row['usercode'])){
        $stmt_user = $link->prepare("SELECT userdesc FROM users WHERE usercode = ? LIMIT 1");
        $stmt_user->execute(array($row['usercode']));
        $user_row = $stmt_user->fetch();
        if($user_row){
            $userdesc = htmlspecialchars($user_row['userdesc'] ?? '', ENT_QUOTES);
        }
    }
    ...
    $xret["html"] .= "<td style='padding:0.5rem;font-size:16px;'>" . $ordernum . "</td>";
    $xret["html"] .= "<td style='padding:0.5rem;font-size:16px;'>" . $docnum . "</td>";
    $xret["html"] .= "<td style='padding:0.5rem;font-size:16px;'>" . $userdesc . "</td>";
    ...
    $xret["html_mobile"] .= "<tr>";
    $xret["html_mobile"] .= "<td style='font-weight:bold;padding:0.5rem;'>Date Uploaded</td>";
    $xret["html_mobile"] .= "<td style='padding:0.5rem;'>" . htmlspecialchars($date_display, ENT_QUOTES) . "</td>";
    $xret["html_mobile"] .= "</tr>";

    $xret["html_mobile"] .= "<tr>";
    $xret["html_mobile"] .= "<td style='font-weight:bold;padding:0.5rem;'>Order Number</td>";
    $xret["html_mobile"] .= "<td style='padding:0.5rem;'>" . $ordernum . "</td>";
    $xret["html_mobile"] .= "</tr>";

    $xret["html_mobile"] .= "<tr>";
    $xret["html_mobile"] .= "<td style='font-weight:bold;padding:0.5rem;'>Docnum</td>";
    $xret["html_mobile"] .= "<td style='padding:0.5rem;'>" . $docnum . "</td>";
    $xret["html_mobile"] .= "</tr>";
