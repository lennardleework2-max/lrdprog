<?php
    ...
    foreach($filter_array as $item){
        ...
        while($rs_main = $stmt_main->fetch()){
            ...
            $pdf->ezPlaceData($xleft+=170,$xtop-9,"<b>In</b>",9 ,'right');
            ...
            while($rs_main2 = $stmt_main2->fetch()){ 
                ...
                if($rs_main2["tranfile2_stkqty"] > 0){
                    $pdf->ezPlaceData($xleft+=170,$xtop,number_format($rs_main2["tranfile2_stkqty"]),9,"right");
                    $in_total+=$rs_main2["tranfile2_stkqty"];
                }else{

                    if ($_POST['txt_output_type'] =='tab')
                    {
                        $pdf->ezPlaceData($xleft+=100,$xtop," ",9,"right");
                    }

                    $rs_main2["tranfile2_stkqty"] = $rs_main2["tranfile2_stkqty"] * - 1;
                    $pdf->ezPlaceData($xleft+=215,$xtop,number_format($rs_main2["tranfile2_stkqty"]),9,"right");
                    $out_total+=$rs_main2["tranfile2_stkqty"];
                }
            ...
            if($_POST['txt_output_type'] =='tab'){
                ...
                $xleft = 500;
                $pdf->ezPlaceData($xleft+71,$xtop-9,"<b>Total:</b>",9 ,'right');
                $pdf->ezPlaceData($xleft+127,$xtop-9,"<b>".number_format($in_total)."</b>",9 ,'right');
                $pdf->ezPlaceData($xleft+170,$xtop-9,"<b>".number_format($out_total)."</b>",9 ,'right');
            }
            ...
            if($_POST['txt_output_type'] =='tab'){
                ...
                $pdf->ezPlaceData(9,$xtop-9," ",9 ,'right');
                $pdf->ezPlaceData(10,$xtop-9," ",9 ,'right');
                $pdf->ezPlaceData(650,$xtop-9,"<b>Balance:</b>",9,'right');
                ...
                $pdf->ezPlaceData($xleft+=85,$xtop-9,"<b>Balance:</b>",9,'right');
                $balance = ($rs_balance['xsum'] + $in_total) - $out_total;
                $pdf->ezPlaceData($xleft+=41,$xtop-9,"<b>".number_format($balance)."</b>",9 ,'right');
            }            
