<?php
    ...
    $xheader_check = false;
        ...
        if($_POST['txt_output_type'] == 'tab'){
            ...
            $pdf->ezPlaceData($xleft, $xtop,"<b>Purchases Order by Doc. Num (Summarized)</b>", 15, 'left' );
            $xtop   -= 15;
    ...
    $xorder = '';
        ...
        (isset($_POST['doc_to']) && !empty($_POST['doc_to'])) ){
            ...
            $xfilter .= " AND purchasesorderfile1.docnum>='".$_POST['doc_from']."' AND purchasesorderfile1.docnum<='".$_POST['doc_to']."'";
        }

        else if(isset($_POST['doc_from']) && !empty($_POST['doc_from'])){
            $xfilter .= " AND purchasesorderfile1.docnum>='".$_POST['doc_from']."'";
        }

        else if(isset($_POST['doc_to']) && !empty($_POST['doc_to'])){

            $xfilter .= " AND purchasesorderfile1.docnum<='".$_POST['doc_to']."'";
        }
        ...
        if(isset($_POST['radio_amount']) && !empty($_POST['radio_amount']) && $_POST['radio_amount'] == 'all_amount'){
            ...
            $xfilter .= " AND (purchasesorderfile1.paydate='' OR purchasesorderfile1.paydate IS NULL)";
        }
        else if(isset($_POST['radio_amount']) && !empty($_POST['radio_amount']) && $_POST['radio_amount']== 'paid'){
            $xfilter .= " AND (purchasesorderfile1.paydate!='' OR purchasesorderfile1.paydate IS NOT NULL)";
        }
    ...
    while($rs_main = $stmt_main->fetch()){    
        ...
        //$pdf->ezPlaceData(1,1,$xcurrentpageno,10,'left');
