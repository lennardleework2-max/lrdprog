<?php
    ...
    $fields = '';
        ...
        if($_POST['txt_output_type'] != 'tab'){
            ...
            //    (isset($_POST['cus_search'])) && !empty($_POST['cus_search'])){
                ...
                $pdf->ezPlaceData($xleft+=60,$xtop,$_POST['date_from'],10,'left');
                $pdf->ezPlaceData($xleft+=60,$xtop,"<b>Date To:</b>",10,'left');
                $pdf->ezPlaceData($xleft+=50,$xtop,$_POST['date_to'],10,'left');
    ...
    (isset($_POST['date_to']) && !empty($_POST['date_to'])) ){
        ...
        $xfilter .= " AND purchasesorderfile1.trndte>='".$xdate_from_filter."'";
    }

    else if(isset($_POST['date_to']) && !empty($_POST['date_to'])){
        $xdate_to_filter = date("Y-m-d", strtotime($_POST['date_to']));
        $xfilter .= " AND purchasesorderfile1.trndte<='".$xdate_to_filter."'";
    }
