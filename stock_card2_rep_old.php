<?php
    ...
    while($rs_main = $stmt_main->fetch()){
        ...
        $pdf->ezPlaceData($xleft+=125,$xtop-9,"<b>Cus./Supplier</b>",9 ,'left');
        ...
        $select_db2 = "SELECT tranfile1.trndte as tranfile1_trndte, tranfile1.trncde as  tranfile1_trncde, tranfile2.docnum as tranfile2_docnum, tranfile2.untprc as tranfile2_untprc, tranfile1.ordernum as tranfile1_ordernum, customerfile.cusdsc as customerfile_cusdsc, supplierfile.suppdsc as supplierfile_suppdsc, tranfile1.orderby as tranfile1_orderby, tranfile2.stkqty as tranfile2_stkqty FROM tranfile2 LEFT JOIN tranfile1 ON tranfile2.docnum = tranfile1.docnum LEFT JOIN itemfile ON itemfile.itmcde =  tranfile2.itmcde LEFT JOIN supplierfile ON tranfile1.suppcde = supplierfile.suppcde LEFT JOIN customerfile ON tranfile1.cuscde = customerfile.cuscde WHERE true ".$xfilter2." AND tranfile2.itmcde='".$rs_main['itmcde']."' ORDER BY tranfile1.trndte ASC, tranfile2.recid ASC";
        ...
        while($rs_main2 = $stmt_main2->fetch()){ 
            ...
            if (isset($_POST['txt_output_type']) && $_POST['txt_output_type']=='tab')
            {
                ...
                $rs_main2["tranfile1_orderby"] = $rs_main2["tranfile1_orderby"];
    
            }else{
                $rs_main2["tranfile1_ordernum"] = trim_str($rs_main2["tranfile1_ordernum"],100,9);
                $supp_or_cus = trim_str($supp_or_cus,70,9);
                $rs_main2["tranfile1_orderby"] = trim_str($rs_main2["tranfile1_orderby"],70,9);
            }
            ...
            $pdf->ezPlaceData($xleft+=95,$xtop,$rs_main2["tranfile1_orderby"],9,"left");
