...
<body>   
    <form method='post' action='logout_btn.php' target='_self' style='display:flex;align-items:center;justify-content:center;height:100vh' name='myform'>
        ...
        <input type="hidden" name="login_hidden" id="login_hidden">
            ...
            <script>
      // Check if user is already logged in by reading cookies
      window.onload = function () {
        const comp_code = getCookie("comp_code");
        const user = getCookie("user");
        const pwd = getCookie("pwd");
        //alert(user);
        if (comp_code) {
            document.getElementById("comp_code").value = comp_code;
        }
        if (user) {
      ...
      function xlogout() {
        setCookie("user", "", -1); // Delete cookie by setting it with a past expiration
    ...
    <script>
        ...
        function login(){
            ...
            setCookie("comp_code", compLO, 365); // Set cookie for 1 day
            setCookie("user", userLO, 365); // Set cookie for 1 day
            setCookie("pwd", passLO, 365); // Set cookie for 1 day
