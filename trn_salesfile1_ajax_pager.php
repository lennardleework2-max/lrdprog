...
$search = false;
    ...
    if(isset($_POST["first_load"]) && $_POST["first_load"]!== "first_load"){
        ...
        if(isset($_POST['salesman_search_h']) && !empty($_POST['salesman_search_h'])){
            $xfilter = " AND tranfile1.salesman_id LIKE '%".$_POST['salesman_search_h']."%'";
            $search = true;
        }
        ...
        if(isset($_POST['paid_search_h']) && !empty($_POST['paid_search_h']) && ($_POST['paid_search_h'] != 'all')){
            ...
            if($_POST['paid_search_h'] == 'paid'){
                $xfilter .= " AND tranfile1.paydate IS NOT NULL AND tranfile1.paydate !=''";
            }else{
                $xfilter .= " AND tranfile1.paydate IS NULL OR tranfile1.paydate =''";
            }
            $search = true;
        }


        if(isset($_POST['paid_salesman_search_h']) && !empty($_POST['paid_salesman_search_h']) && ($_POST['paid_salesman_search_h'] != 'all')){

            if($_POST['paid_salesman_search_h'] == 'paid'){
                $xfilter .= " AND tranfile1.paydate_salesman IS NOT NULL AND tranfile1.paydate_salesman !=''";
            }else{
                $xfilter .= " AND tranfile1.paydate_salesman IS NULL OR tranfile1.paydate_salesman =''";
            }
            $search = true;
        }
...
$select_db_xtotal = "SELECT count(*) as rec_count FROM tranfile1 LEFT JOIN customerfile ON 
tranfile1.cuscde = customerfile.cuscde LEFT JOIN  mf_buyers ON
mf_buyers.buyer_id = tranfile1.buyer_id 
...
$select_db="SELECT tranfile1.img_filename as 'img_filename', mf_buyers.buyer_name as mf_buyer_buyer_name, tranfile1.buyer_id, tranfile1.shipto as tranfile1_shipto,tranfile1.cuscde as tranfile1_cuscde,tranfile1.docnum as tranfile1_docnum,
...
WHERE true AND trncde='".$trncde."' ".$xfilter." LIMIT ".$xlimit." OFFSET ".$xoffset;
$xret["sql"] = $select_db;
...
while($row_main = $stmt->fetch()){
    ...
    $xret["html"] .= "<tr class='tr_striped'>";
        $xret["html"] .= "<td data-label='Username' style='width:80%;font-size:20px;".$xtop_border_action."'>
            ...
            border-collapse:collapse' id='trn_sales_table'>
                    ...
                    if($row_main['img_filename'] !== "" && !empty($row_main['img_filename'])){
                        $xret["html"].= "
                            <tr>
                                <td colspan='4' style='padding:0.3rem;text-align:center;border-right:2px solid gray;padding:0.3rem'>
                                    <img src='images_sales/".$row_main['img_filename']."' style='width:300px;height:240px'>
                                </td>
                            </tr>
    ...
    $xret["html_mobile"] .= "<tr>";
        ...
        $xret["html_mobile"] .= "<td>".$row_main['mf_buyer_buyer_name']."</td>";
    $xret["html_mobile"] .= "</tr>";

    $xret["html_mobile"] .= "<tr>";
        $xret["html_mobile"] .= "<td style='font-weight:bold'>Pay Date</td>";
        $xret["html_mobile"] .= "<td>".$row_main['tranfile1_paydate']."</td>";
    $xret["html_mobile"] .= "</tr>";
    ...
    if($row_main['img_filename'] !== "" && !empty($row_main['img_filename'])){
        $xret["html_mobile"] .= "<tr>";
            $xret["html_mobile"] .= "<td colspan='2' >";
            $xret["html_mobile"] .= "
                                        <div style='display:flex;flex-direction:column'>
                                        <label class='fw-bold mb-2'>Image:</label>
                                        <img src='images_sales/".$row_main['img_filename']."' style='width:150px;height:150px'>
                                        </div>
            ";
            $xret["html_mobile"] .= "</td>";
        $xret["html_mobile"] .= "</tr>";
