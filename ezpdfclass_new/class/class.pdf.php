...
//        $fontObj = $this->numObj;
        ...
        $fp = fopen($fbfile,'rb');
        $tmp = function_exists('get_magic_quotes_runtime') ? get_magic_quotes_runtime() : 0;
        //if (function_exists('set_magic_quotes_runtime')) { set_magic_quotes_runtime(0); }
        $data = fread($fp,filesize($fbfile));
        //if (function_exists('set_magic_quotes_runtime')) { set_magic_quotes_runtime($tmp); }
        fclose($fp);
...
function setCurrentFont(){
  ...
  if (strlen($this->currentTextState)
    ...
    if (isset($this->fonts[$this->currentFont]['fontNum'])) {
        ...
        $this->currentFontNum = 0; // or a safe default
        error_log("Warning: Font not found for key '{$this->currentFont}'");
    }
  }
}
...
function addImage(&$img,$x,$y,$w=0,$h=0,$quality=75){
  ...
  $tmpDir = sys_get_temp_dir(); // Automatically selects correct temp dir
  $tmpName = @tempnam($tmpDir, 'img'); // Suppress notice if needed
