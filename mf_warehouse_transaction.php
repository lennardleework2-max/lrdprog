...
if ($edit_recid !== '') {
    ...
    if ($edit_row) {
        ...
        $form_state = form_state_from_row($edit_row);
        $floor_id = $form_state['source_floor_id'];
        $warehouse_id = warehouse_id_from_floor($floor_map, $floor_id);
        $selected_warehouse = ($warehouse_id !== '' && isset($warehouse_by_id[$warehouse_id])) ? $warehouse_by_id[$warehouse_id] : false;
        $selected_floor = ($floor_id !== '' && isset($floor_map[$floor_id])) ? $floor_map[$floor_id] : false;
        $has_context = ($selected_warehouse !== false && $selected_floor !== false);
        $mode = 'edit';
    } else {
        $page_warning = 'The selected transaction was not found for the chosen warehouse and floor.';
        $mode = 'view';
    }
}
...
if (isset($_POST['form_action']) && ($_POST['form_action'] === 'create' || $_POST['form_action'] === 'update')) {
    ...
    $form_state = array(
        ...
        'source_floor_id' => isset($_POST['source_floor_id']) ? trim((string)$_POST['source_floor_id']) : $floor_id,
                            ...
                            <?php else: ?>
                                <div class="row g-3">
                                    <?php foreach ($transactions as $transaction): ?>
                                        <?php
                                            $source_label = trim($transaction['source_warehouse_name'] . ' / ' . floor_label($transaction), ' /');
                                            $related_label = '';
                                            if (!empty($transaction['related_floor_name']) || !empty($transaction['related_floor_no']) || !empty($transaction['related_warehouse_name'])) {
                                                $related_label = trim($transaction['related_warehouse_name'] . ' / ' . floor_label(array(
                                                    'floor_name' => $transaction['related_floor_name'],
                                                    'floor_no' => $transaction['related_floor_no']
                                                )), ' /');
                                            }
                                            $staff_label = trim((string)$transaction['fname'] . ' ' . (string)$transaction['lname']);
                                            $edit_url = "mf_warehouse_transaction_entry.php?warehouse_id=" . urlencode($transaction['source_warehouse_id']) . "&floor_id=" . urlencode($transaction['floor_id']) . "&edit=" . urlencode($transaction['recid']);
                                            $view_payload = array(
                                                'movement_id' => $transaction['movement_id'],
                                                'movement_date' => date('Y-m-d H:i', strtotime($transaction['movement_date'])),
                                                'movement_type' => $transaction['movement_type'],
                                                'item_label' => item_label($transaction),
                                                'qty' => qty_display($transaction['qty']),
                                                'stkqty' => qty_display($transaction['stkqty']),
                                        ...
                                        <div class="col-12 col-xl-6">
                                            <div class="transaction-summary-card">
                                                <div class="d-flex flex-wrap justify-content-between align-items-start gap-2 mb-2">
                                                    <div class="small text-muted"><?php echo h(date('Y-m-d H:i', strtotime($transaction['movement_date']))); ?></div>
                                                    <span class="badge bg-secondary"><?php echo h($transaction['movement_type']); ?></span>
                                                </div>
                                                <div class="transaction-id"><?php echo h($transaction['movement_id']); ?></div>
                                                ...
                                                <div class="transaction-meta">
                                                    ...
                                                    <div class="transaction-meta-block">
                                                        ...
                                                        <span class="transaction-meta-value <?php echo ((float)$transaction['stkqty'] >= 0) ? 'text-success' : 'text-danger'; ?>"><?php echo h(qty_display($transaction['stkqty'])); ?></span>
                                                    </div>
                                                    <div class="transaction-meta-block">
                                                ...
                                                <div class="d-flex flex-wrap gap-2 mt-3">
                                                    <button
                                                        ...
                                                        data-edit-url="<?php echo h($edit_url); ?>"
                                                        onclick="openTransactionView(JSON.parse(this.dataset.transaction), this.dataset.editUrl)">
                                                        View
                                                    </button>
                                                    <form method="post" onsubmit="return confirm('Delete this transaction? Transfer rows will delete the matching pair when found.');">
                                                        ...
                                                        <input type="hidden" name="recid" value="<?php echo h($transaction['recid']); ?>">
                    ...
                    <div class="form-card p-3 p-md-4 mb-3">
                        ...
                        <form method="post" id="movement_form">
                            ...
                            <div class="row g-3">
                                    ...
                                    <select class="form-select" id="floor_id" name="floor_id">
                                        ...
                                        <?php foreach ($source_floors as $floor_option): ?>
                                            <option value="<?php echo h($floor_option['warehouse_floor_id']); ?>" <?php echo ($floor_id === $floor_option['warehouse_floor_id']) ? 'selected' : ''; ?>>
                                                <?php echo h(floor_label($floor_option)); ?>
                                            </option>
                                        <?php endforeach; ?>
                                    </select>
                                </div>

                                <div class="col-12 col-lg-6" id="transfer_destination_warehouse_wrap" style="display:none;">
                                ...
                                <div class="col-12 col-lg-6" id="transfer_destination_wrap" style="display:none;">
                                ...
                                <div class="col-12 col-lg-4">
                                    ...
                                    <div class="mt-2" id="transfer_stock_wrap" style="display:none;">
                                        <div class="stock-indicator" id="transfer_stock_box">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="input-group mb-3">
                    <input type="text" class="form-control" id="search_item_text" placeholder="Enter item name or code" autocomplete="off">
                    <button type="button" class="btn btn-success" id="search_item_btn"><i class="fas fa-search"></i></button>
                </div>
                <div id="item_search_results" class="table-responsive">
                    <div class="text-muted">Search an item to start.</div>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal fade" id="transactionViewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered modal-lg modal-dialog-scrollable">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <?php if ((int)$edit_crud === 1): ?>
                    <a href="#" class="btn btn-outline-primary" id="view_edit_link">Edit Transaction</a>
                <?php endif; ?>
...
<script>
    (function () {
        ...
        var transferDestinationWarehouseWrap = document.getElementById('transfer_destination_warehouse_wrap');
        var transferDestinationWrap = document.getElementById('transfer_destination_wrap');
        ...
        var searchItemBtn = document.getElementById('search_item_btn');
        var searchItemText = document.getElementById('search_item_text');
        var itemSearchResults = document.getElementById('item_search_results');
        ...
        function optionLabel(floor, includeWarehouseName) {
            ...
            if (floor.floor_no) {
                parts.push('Floor ' + floor.floor_no);
            }
        ...
        function updateTransferUI() {
            ...
            if (transferDestinationWarehouseWrap) {
                transferDestinationWarehouseWrap.style.display = (currentAction === 'TRANSFER') ? '' : 'none';
            }
            if (transferDestinationWrap) {
                transferDestinationWrap.style.display = (currentAction === 'TRANSFER') ? '' : 'none';
            }
            ...
            if (shouldShowStockPreview()) {
                ...
                if (transferStockBox) {
                    transferStockBox.innerHTML = 'Select an item to view current stock.';
                }
            }
        }
        ...
        function fetchTransferStock() {
            ...
            transferStockBox.innerHTML = 'Loading current stock...';
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (response && response.status === 1) {
                        transferStockBox.innerHTML = response.html;
                        updateQuantityLimit(parseFloat(response.available_stock));
                    } else {
                        updateQuantityLimit(null);
                        transferStockBox.innerHTML = 'Unable to load current stock.';
                    }
                },
                error: function () {
                    updateQuantityLimit(null);
        ...
        function searchItems() {
            var searchText = searchItemText ? searchItemText.value : '';

            if (!itemSearchResults) {
                return;
            }

            itemSearchResults.innerHTML = '<div class="text-muted">Searching items...</div>';
            ...
            $.ajax({
                ...
                dataType: 'json',
                data: {
                    event_action: 'search_item',
                    search_item: searchText
                },
                success: function (response) {
                    if (response && response.status === 1) {
                        itemSearchResults.innerHTML = response.html;
                    } else {
                        itemSearchResults.innerHTML = '<div class="text-danger">Unable to search items.</div>';
                    }
                },
                error: function () {
                    itemSearchResults.innerHTML = '<div class="text-danger">Unable to search items.</div>';
        ...
        window.openTransactionView = function (data, editUrl) {
            ...
            var editLink = document.getElementById('view_edit_link');
            if (editLink) {
        ...
        if (searchItemBtn) {
            searchItemBtn.addEventListener('click', searchItems);
        }

        if (searchItemText) {
            searchItemText.addEventListener('keydown', function (event) {
                if (event.key === 'Enter') {
