<?php
    ...
    $fields = '';
        ...
        if(!isset($_POST['customerfile_cusdsc']) || empty($_POST['customerfile_cusdsc'])){
            $_POST['customerfile_cusdsc'] = '';
        }
        ...
        if(!isset($_POST['shipping_method_dr']) || empty($_POST['shipping_method_dr'])){
            $_POST['shipping_method_dr'] = '';
        }
        ...
        */
        ...
        $select_db_docnum="SELECT *, tranfile1.docnum as tranfile1_docnum,
                                     ...
                                     tranfile1.orderby as tranfile1_orderby,
                                     mf_buyers.buyer_name as mf_buyers_buyername,
                                     mf_routes.route_desc as mf_route_route_desc,
                                     tranfile1.shipto as tranfile1_shipto FROM tranfile1 LEFT
        JOIN customerfile ON tranfile1.cuscde = customerfile.cuscde 
        LEFT JOIN mf_buyers ON tranfile1.buyer_id = mf_buyers.buyer_id 
        LEFT JOIN mf_routes ON tranfile1.route_id = mf_routes.route_id 
        WHERE tranfile1.recid=".$_POST['recid_hidden']."";
        ...
        //$pdf->ezPlaceData($xleft, $xtop+25,"Route:", 10, 'left' );
        $pdf->setColor(0, 0, 0); // RGB values as floats: red = 1, green = 0, blue = 0
        if(!(empty($rs_main_docnum['mf_route_route_desc']) || $rs_main_docnum['mf_route_route_desc'] == "-")){
            $pdf->ezPlaceData($xleft+200, $xtop+25, "<b>" . $rs_main_docnum['mf_route_route_desc']. "</b>", 20, 'right');
        }

        $pdf->setColor(1, 0, 0); // RGB values as floats: red = 1, green = 0, blue = 0
        $pdf->ezPlaceData($xleft, $xtop+15,"STRICT 90 DAYS ONLY ", 10, 'left' );
        $pdf->setColor(0, 0, 0); // RGB values as floats: red = 1, green = 0, blue = 0
        ...
        $pdf->setColor(0, 0, 0); // RGB values as floats: red = 1, green = 0, blue = 0
        $pdf->ezPlaceData($xleft+30, $xtop+12, $formatted_date, 10, 'left' );
    ...
    $pdf->setColor(1, 0, 0); // red
    $pdf->addText($xleft-=35, 100, 12, 'Account Name:');
    $pdf->setColor(0, 0, 0); // red
    ...
    // Signature fields on the right side of the black box
    $xsig_left = 360;
    $pdf->setColor(1, 0, 0); // red
    $pdf->addText($xsig_left, 115, 10, 'Packed By:');
    $pdf->setColor(1, 0, 0); // red
    $pdf->addText($xsig_left, 100, 10, 'Prepared By:');
    $pdf->setColor(1, 0, 0); // red
    $pdf->addText($xsig_left, 85, 10, 'Approved By:');
    ...
    $xleft = 25;
        ...
        $pdf->setColor(0, 0, 0); // RGB values as floats: red = 1, green = 0, blue = 0
        $pdf->ezPlaceData($xleft+115, $xtop+30,"DUPLICATE COPY ", 10, 'left' );
        $pdf->ezPlaceData($xleft+115, $xtop+15,"STRICT 90 DAYS ONLY ", 10, 'left' );
        $pdf->setColor(0, 0, 0); // RGB values as floats: red = 1, green = 0, blue = 0
    ...
    function cleanTextForPDF($text) {
        ...
        // Convert to ASCII-safe encoding (ezPDF works best with this)
    ...
    while (ob_get_level()) {
        ...
        // Allow preview AND set proper download name
    header('Content-Type: application/pdf');
    header('Content-Disposition: inline; filename="delivery_receipt.pdf"'); // <-- this enables preview
