...
$search = false;
    ...
    if(isset($_POST["first_load"]) && $_POST["first_load"]!== "first_load"){
        ...
        if(isset($_POST['orderby_search_h']) && !empty($_POST['orderby_search_h'])){
            $xfilter = "AND salesorderfile1.orderby LIKE '%".$_POST['orderby_search_h']."%'";
            $search = true;
        }

        if(isset($_POST['docnum_search_h']) && !empty($_POST['docnum_search_h'])){
            $xfilter .= " AND salesorderfile1.docnum LIKE '%".$_POST['docnum_search_h']."%'";
            $search = true;
        }

        if(isset($_POST['ordernum_file_search_h']) && !empty($_POST['ordernum_file_search_h'])){
            $xfilter .= " AND salesorderfile1.ordernum LIKE '%".$_POST['ordernum_file_search_h']."%'";
            $search = true;
        }
        ...
        (isset($_POST['to_search_h']) && !empty($_POST['to_search_h'])) ){
            ...
            $xfilter .= " AND salesorderfile1.trndte>='".$_POST['from_search_h']."' AND salesorderfile1.trndte<='".$_POST['to_search_h']."'";
            $search = true;
        }

        else if(isset($_POST['from_search_h']) && !empty($_POST['from_search_h'])){
            $_POST['from_search_h'] = date("Y-m-d", strtotime($_POST['from_search_h']));
            $xfilter .= " AND salesorderfile1.trndte>='".$_POST['from_search_h']."'";
            $search = true;
        }

        else if(isset($_POST['to_search_h']) && !empty($_POST['to_search_h'])){
            $_POST['to_search_h'] = date("Y-m-d", strtotime($_POST['to_search_h']));
            $xfilter .= " AND salesorderfile1.trndte<='".$_POST['to_search_h']."'";
            $search = true;
        }
        ...
        (isset($_POST['to_ordered_search_h']) && !empty($_POST['to_ordered_search_h'])) ){
            ...
            $xfilter .= " AND DATE(salesorderfile1.file_created_date)>='".$_POST['from_ordered_search_h']."' AND DATE(salesorderfile1.file_created_date)<='".$_POST['to_ordered_search_h']."'";
            $search = true;
        }

        else if(isset($_POST['from_ordered_search_h']) && !empty($_POST['from_ordered_search_h'])){
            $_POST['from_ordered_search_h'] = date("Y-m-d", strtotime($_POST['from_ordered_search_h']));
            $xfilter .= " AND DATE(salesorderfile1.file_created_date)>='".$_POST['from_ordered_search_h']."'";
            $search = true;
        }

        else if(isset($_POST['to_ordered_search_h']) && !empty($_POST['to_ordered_search_h'])){
            $_POST['to_ordered_search_h'] = date("Y-m-d", strtotime($_POST['to_ordered_search_h']));
            $xfilter .= " AND DATE(salesorderfile1.file_created_date)<='".$_POST['to_ordered_search_h']."'";
            $search = true;
        }        
        ...
        if(isset($_POST['waybill_num_search_h']) && !empty($_POST['waybill_num_search_h'])){
            $xfilter .= " AND salesorderfile1.waybill_number LIKE '%".$_POST['waybill_num_search_h']."%' ";
            $search = true;
        }        

        if(($_POST['sortby_1_field_h']!=='none' && !empty($_POST['sortby_1_field_h'])) && ($_POST['sortby_2_field_h']!=='none' && !empty($_POST['sortby_2_field_h']))){
            $xfilter.=" ORDER BY salesorderfile1.".$_POST['sortby_1_field_h']." ".$_POST['sortby_1_order_h'].", salesorderfile1.".$_POST['sortby_2_field_h']." ".$_POST['sortby_2_order_h']."";
            $search = true;
        }

        else if($_POST['sortby_1_field_h']!=='none' && !empty($_POST['sortby_1_field_h'])){
            $xfilter.=" ORDER BY salesorderfile1.".$_POST['sortby_1_field_h']." ".$_POST['sortby_1_order_h']."";
            $search = true;
        }
        else if($_POST['sortby_2_field_h']!=='none' && !empty($_POST['sortby_2_field_h'])){
            $xfilter.=" ORDER BY salesorderfile1.".$_POST['sortby_2_field_h']." ".$_POST['sortby_2_order_h']."";
            $search = true;
        }
    }

    if($search == false){
        $xfilter =" ORDER BY salesorderfile1.docnum DESC";
    }

$select_db_xtotal = "SELECT count(*) as rec_count FROM salesorderfile1 LEFT JOIN customerfile ON salesorderfile1.cuscde = customerfile.cuscde WHERE true AND trncde='".$trncde."' AND salesorderfile1.docnum NOT LIKE '%BOM%' ".$xfilter;
...
$select_db="SELECT salesorderfile1.waybill_number as 'so_waybill_number', salesorderfile1.file_ordernum as 'file_ordernum', salesorderfile1.file_created_date as 'file_created_date', salesorderfile1.shipto as salesorderfile1_shipto,salesorderfile1.cuscde as salesorderfile1_cuscde,salesorderfile1.docnum as salesorderfile1_docnum,
...
salesorderfile1.cuscde = customerfile.cuscde WHERE true AND trncde='".$trncde."' AND salesorderfile1.docnum NOT LIKE '%BOM%' ".$xfilter." LIMIT ".$xlimit." OFFSET ".$xoffset;
$xret["sql"] = $select_db;
...
while($row_main = $stmt->fetch()){
    ...
    if(empty($row_main['salesorderfile1_orderby'])){
        $row_main['salesorderfile1_orderby']=  '&nbsp;';
    }
    ...
    $arr_alert_del2 = '';
    $select_db2="SELECT * FROM salesorderfile2 WHERE docnum='".$row_main['salesorderfile1_docnum']."'";
    $stmt2	= $link->prepare($select_db2);
    $stmt2->execute();
    $xcounter3 = 0;
    while($row_main2 = $stmt2->fetch()){

        $select_db3="SELECT * FROM tranfile2 WHERE so_recid='".$row_main2['recid']."'";
    ...
    $xret["html_mobile"] .= "<tr>";
        ...
        $xret["html_mobile"] .= "<td>".$row_main['file_ordernum']."</td>";
    $xret["html_mobile"] .= "</tr>";
    ...
    $xret["html_mobile"] .= "<tr>";
        ...
        $xret["html_mobile"] .= "<td>".$row_main['salesorderfile1_orderby']."</td>";
    $xret["html_mobile"] .= "</tr>";
