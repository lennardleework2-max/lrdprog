<?php
    ...
    $xchecker_3_header = false;
        ...
        if($_POST['txt_output_type'] == 'tab'){
            ...
            $pdf->ezPlaceData($xleft, $xtop,"<b>Sales Order by Ordered Date (With Item)</b>", 15, 'left' );
            $xtop   -= 15;
        ...
        $xfilter .= " AND DATE(salesorderfile1.file_created_date)>='".$xdate_from_filter."' AND DATE(salesorderfile1.file_created_date)<='".$xdate_to_filter."'";
        ...
        if($_POST['txt_output_type'] != 'tab'){
            ...
            $tab_headers = "Doc. Num.\tOrdered Date\tUpload Date\tOrdered By\tPlatform/Item\tQuantity\tUOM\tUnit Price\tTotal\n";
            echo $tab_headers;
        }     
        ...
        if($_POST['txt_output_type'] != 'tab'){
            ...
            $pdf->ezPlaceData($xleft+=155,$xtop,"<b>Quantity</b>",10,'right');
            $pdf->ezPlaceData($xleft+=10,$xtop,"<b>UOM</b>",10,'left');
            $pdf->ezPlaceData($xleft+=115,$xtop,"<b>Unit Price</b>",10,'right');
            $pdf->ezPlaceData($xleft+=85,$xtop,"<b>Total</b>",10,'right');
        }       
    ...
    (isset($_POST['date_to']) && !empty($_POST['date_to'])) ){
        ...
        $xfilter .= " AND DATE(salesorderfile1.file_created_date)>='".$_POST['date_from']."' AND DATE(salesorderfile1.file_created_date)<='".$_POST['date_to']."'";
    }

    else if(isset($_POST['date_from']) && !empty($_POST['date_from'])){
        $_POST['date_from'] = date("Y-m-d", strtotime($_POST['date_from']));
        $xfilter .= " AND DATE(salesorderfile1.file_created_date)>='".$_POST['date_from']."'";
    }

    else if(isset($_POST['date_to']) && !empty($_POST['date_to'])){
        $_POST['date_to'] = date("Y-m-d", strtotime($_POST['date_to']));
        $xfilter .= " AND DATE(salesorderfile1.file_created_date)<='".$_POST['date_to']."'";
    }
    ...
    while($rs_main = $stmt_main->fetch()){ 
        ...
        // $pdf->ezPlaceData($xleft+=135,$xtop,$rs_main["salesorderfile1_paydate"],9,"left");
        // $pdf->ezPlaceData($xleft+=85,$xtop,$rs_main["salesorderfile1_paydetails"],9,"left");
        // $pdf->ezPlaceData($xleft+215,$xtop,number_format($rs_main["salesorderfile1_trntot"],"2"),9,"right");
        ...
        while($rs_main2 = $stmt_main2->fetch()){  
            ...
            if($_POST['txt_output_type'] == 'tab'){
                ...
                $rs_main2["itmqty"]. "\t" .
                xls_safe_text(isset($rs_main2["unmdsc"]) ? $rs_main2["unmdsc"] : ''). "\t" .
                $rs_main2["untprc"] . "\t" .
                $rs_main2["extprc"] . "\n";

                echo $tab_output;
            }else{
                $pdf->ezPlaceData($xleft+=220,$xtop+$xcount_total_itmheight,$rs_main2["itmqty"],9,"right");
                $pdf->ezPlaceData($xleft+=10,$xtop+$xcount_total_itmheight,trim_str(isset($rs_main2["unmdsc"]) ? $rs_main2["unmdsc"] : '',60,9),9,"left");
                $pdf->ezPlaceData($xleft+=115,$xtop+$xcount_total_itmheight,number_format($rs_main2["untprc"],"2"),9,"right");
                $pdf->ezPlaceData($xleft+=85,$xtop+$xcount_total_itmheight,number_format($rs_main2["extprc"],"2"),9,"right");
            }              
        ...
        if($_POST['txt_output_type'] == 'tab'){
            ...
            $pdf->ezPlaceData($xleft+=620,$xtop+=5,"<b>TOTAL:</b>",9,"left");
    ...
    if($_POST['txt_output_type'] == 'tab'){
        ...
        $pdf->ezPlaceData($xleft+=600,$xtop+=5,"<b>GRAND TOTAL:</b>",8,"left");
